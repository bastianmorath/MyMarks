...
- (void)viewDidLoad
{
    ...
    // Attachments
    self.attachmentsView = [[UIView alloc] initWithFrame:CGRectMake(20, CGRectGetMaxY(self.textView.frame) - 100, CGRectGetWidth(self.view.frame) - 30, 22)];
    self.attachmentsView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    [scrollView addSubview:self.attachmentsView];
    
    // Attachment button
    self.attachmentsButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    self.attachmentsButton.titleLabel.textColor = [UIColor blackColor];
    self.attachmentsButton.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:17];
    ...
    // Preview image
    float imageSize = 100;
    self.attachmentsImageView = [[UIImageView alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.attachmentsView.frame) - imageSize, -(imageSize -22), imageSize, imageSize)];
...
- (void)onKeyboard:(NSNotification*)notification
{
    ...
    tvFrame.size.height = CGRectGetHeight(self.textView.superview.bounds) - (CGRectGetHeight(keyboardRect) + CGRectGetMinY(tvFrame) + topOffset) - CGRectGetHeight(attachFrame) - 20;
...
- (void)showButtons
{
    if (self.navigationItem.leftBarButtonItem == nil && self.navigationController.viewControllers.count == 2) {
